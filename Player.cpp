...
Player::Player() : playerSpeed(1.0f),maxFireRate(600),fireRateTimer(0){
...
void Player::Update(double deltaTime,Skeleton& skeleton,sf::Vector2f& mousePosition) {
    ...
    if(sf::Keyboard::isKeyPressed(sf::Keyboard::W)) {
        sprite.setPosition(position + sf::Vector2f(0,-1)* playerSpeed * (float)deltaTime);//or (position + sf::Vector2f(0,-10))
    }
 ...
 //------------------------------------Bullet---------------------------------
    ...
    if(sf::Mouse::isButtonPressed(sf::Mouse::Button::Left) && fireRateTimer >= maxFireRate) {
        ...
        bullets[i].Initialize(mousePosition,sprite.getPosition(),0.5f);
